		...
		if (token) {
			try {
				...
				if (user) {
					...
					// Check if data is cached related to this Query
					const cachedPosts = await client.get(user._id);
					...
					// else, respond and update the cache to store the data
					// get following posts
					...
					res.status(200).json(posts);
					client.set(user._id, JSON.stringify(posts));
				} else {
